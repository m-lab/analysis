...
SELECT
  ...
  ROUND(100*COUNTIF( LENGTH(client.Geo.ContinentCode)=2 ) / count(*)) AS OkContinent,
  ...
  ROUND(100*COUNTIF( client.Geo.MetroCode>0 ) / count(*)) AS OkMetroCode,
  ROUND(100*COUNTIF( LENGTH(client.Geo.City)>0 ) / count(*)) AS OkCity,
  ROUND(100*COUNTIF( client.Geo.AreaCode>0 ) / count(*)) AS OkAreaCode,
  ROUND(100*COUNTIF( LENGTH(client.Geo.PostalCode)>0 ) / count(*)) AS OkPostalCode,
  ROUND(100*COUNTIF( client.Geo.AccuracyRadiusKm > 0.0 ) / count(*)) AS OkRadius,
